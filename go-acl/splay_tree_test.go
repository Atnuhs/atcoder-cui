...
func TestSplayTree_BasicOperations(t *testing.T) {
	...
	// 検索テスト
	if value, found := tree.Find("banana"); !found || value != 2 {
		t.Errorf("Expected to find banana with value 2, got value=%d, found=%v", value, found)
	}
	
	if _, found := tree.Find("orange"); found {
	...
	if _, found := tree.Find("banana"); found {
...
func TestSplayTree_IntegerKeys(t *testing.T) {
	...
	for i, expected := range expectedKeys {
		...
		if inOrder[i].Key != expected {
			t.Errorf("Expected key %d at index %d, got %d", expected, i, inOrder[i].Key)
		}
		if inOrder[i].Value != values[expected] {
			t.Errorf("Expected value %s for key %d, got %s", values[expected], expected, inOrder[i].Value)
		}
	}
}
...
func TestSplayTree_UpdateValue(t *testing.T) {
	...
	if value, found := tree.Find("key"); !found || value != 100 {
	...
	if value, found := tree.Find("key"); !found || value != 200 {
