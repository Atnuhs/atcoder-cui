...
func ExampleSplayTreeBasicUsage() {
	...
	// 検索
	if value, found := tree.Find("banana"); found {
		fmt.Printf("banana: %d\n", value)
	}
...
func ExampleSplayTreeWithStructs() {
	...
	// ID検索
	if person, found := tree.Find(1002); found {
...
func ExampleSplayTreeFloatKeys() {
	...
	tree := NewSplayTree[float64, string]()
	...
	// 特定の温度
	if status, found := tree.Find(37.5); found {
		fmt.Printf("37.5℃: %s\n", status)
	}
...
func ExampleSplayTreeCompetitiveProgramming() {
	...
	// 特定のx座標の点を検索
	if point, found := tree.Find(3); found {
